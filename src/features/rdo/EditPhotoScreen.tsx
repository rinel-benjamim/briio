...
const MOCK_PHOTOS_DATA: Record<string, PhotoData> = {
  "1": {
    caption: "Frente — Bloco A",
    location: "Bloco A — Entrada",
    type: "execucao",
    uri: null,
  },
  "2": {
    caption: "Alvenaria — Piso 2",
    location: "Bloco A — Piso 2",
    type: "execucao",
    uri: null,
  },
  "3": {
    caption: "Revestimento — Piso 1",
  ...
  "5": {
    caption: "Equipamentos no local",
    location: "Área de serviço",
    type: "equipamento",
    uri: null,
  },
  "6": {
    caption: "Estado geral da obra",
    location: "Vista geral",
    type: "estado_obra",
    uri: null,
  },
};
...
export default function EditPhotoScreen() {
  ...
  const data = MOCK_PHOTOS_DATA[photoId || "1"];

  const [photoUri, setPhotoUri] = useState<string | null>(data?.uri || null);
  const [caption, setCaption] = useState(data?.caption || "");
  const [location, setLocation] = useState(data?.location || "");
  const [photoType, setPhotoType] = useState<PhotoType>(data?.type || "execucao");
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.topNav, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.navTitle}>Editar fotografia</Text>
