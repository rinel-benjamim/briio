...
const MOCK_WORKFORCE_DATA: Record<string, { role: string; people: number; hoursPerPerson: number; observation: string }> = {
...
export default function EditWorkforceScreen() {
  ...
  const data = MOCK_WORKFORCE_DATA[workforceId || "1"];

  const [role, setRole] = useState(data?.role || "");
  const [customRole, setCustomRole] = useState("");
  const [peopleCount, setPeopleCount] = useState(data?.people || 1);
  const [hoursPerPerson, setHoursPerPerson] = useState(data?.hoursPerPerson || 8);
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.topNav, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.navTitle}>Editar função</Text>
          ...
          <View style={styles.field}>
            ...
            >
              <Text style={[styles.dropdownText, !role && styles.dropdownPlaceholder]}>
                {isCustomRole ? customRole || "Inserir nome da função" : role || "Selecione uma função"}
              </Text>
