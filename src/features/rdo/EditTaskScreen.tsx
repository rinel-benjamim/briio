...
const MOCK_TASKS_DATA: Record<string, { description: string; location: string; quantity: number; unit: string; status: StatusOption; observation: string; progress: number }> = {
  "1": { description: "Execução de alvenaria", location: "Piso 2 — Bloco A", quantity: 120, unit: "m²", status: "em_execucao", observation: "", progress: 65 },
  "2": { description: "Assentamento de revestimento", location: "Piso 1 — Bloco B", quantity: 85, unit: "m²", status: "concluida", observation: "", progress: 100 },
};
...
export default function EditTaskScreen() {
  ...
  const data = MOCK_TASKS_DATA[taskId || "1"];

  const [description, setDescription] = useState(data?.description || "");
  const [location, setLocation] = useState(data?.location || "");
  const [quantity, setQuantity] = useState(data?.quantity || 120);
  const [unit, setUnit] = useState(data?.unit || "m²");
  const [status, setStatus] = useState<StatusOption>(data?.status || "em_execucao");
  const [observation, setObservation] = useState(data?.observation || "");
  const [progress, setProgress] = useState(data?.progress || 65);
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.topNav, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.navTitle}>Editar atividade</Text>
