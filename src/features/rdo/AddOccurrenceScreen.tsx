...
export default function AddOccurrenceScreen() {
  ...
  const [timeDate, setTimeDate] = useState(new Date(2026, 7, 12, 14, 20));
  const [showTimePicker, setShowTimePicker] = useState(false);
  const [location, setLocation] = useState("");
  const [description, setDescription] = useState("");
  const [impact, setImpact] = useState<ImpactOption>("impacto_relevante");
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.topNav, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.navTitle}>Registar ocorrência</Text>
